      ...
      book.spine.get = function(target: any) {
        ...
        if (!t && typeof target === 'string') {
          for (let i = 0; i < this.spineItems.length; i++) {
            if (this.spineItems[i].href === target || 
                this.spineItems[i].href.endsWith(target)) {
              return this.spineItems[i]
            }
          }
        }
        return t
...
const displayFallback = () => {
  ...
  rendition.display().catch(err => {
    ...
    // last resort try to get first spine item
    if (book && book.spine && book.spine.items && book.spine.items.length > 0) {
      ...
      if (firstItem && firstItem.href) {
        rendition!.display(firstItem.href).catch(finalErr => {
