  ...
  setup() {
    ...
    const loadLibrary = async (): Promise<void> => {
      ...
      try {
        ...
        books.value = []; // Ensure books is at least an empty array
      } finally {
        loading.value = false;
      }
    };
    ...
    const openReader = async (book: Book): Promise<void> => {
      try {
      ...
      try {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          try {
            ...
            books.value = [...books.value, importedBook];
            importedCount++;
    ...
    const handleBookDeleted = (bookId: string): void => {
      ...
      const deletedBook = books.value.find(b => b.id === bookId);
