...
const loadBook = async (): Promise<void> => {
  ...
  try {
    ...
    const book: Book = await loadBookFromIndexedDB(bookId);
...
const getRendition = (renditionObj: any): void => {
  ...
  renditionObj.on("relocated", (location) => {
...
const onTocChange = (tocData): void => {
  toc.value = tocData.map((i) => ({ ...i, expansion: false }));
};
...
XMLHttpRequest.prototype.open = function (
  ...
  if (bookData.value && requestUrl === bookData.value) {
