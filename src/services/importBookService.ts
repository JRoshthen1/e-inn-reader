...
export class ImportBookService {
   ...
   */
  async loadLocalBooks(): Promise<Book[]> {
    try {
      ...
      return new Promise<Book[]>((resolve, reject) => {
        ...
        request.onsuccess = () => {
          const localBooks = request.result.map((book: Book) => ({
            ...book,
            isLocal: true
          }));
          resolve(localBooks);
   ...
   */
  async storeBook(book: Book, blob?: Blob): Promise<string> {
    try {
      const db = await initializeDB();
   ...
   */
  async bookExistsInDB(book: Book): Promise<boolean> {
    try {
      const db = await initializeDB();
   ...
   * Import a local file
   */
  async importLocalFile(file: File): Promise<Book> {
    try {
